...
import { 
  ...
  Database, CheckCircle2, Loader2, Plus, RefreshCcw, Check, Cloud
} from 'lucide-react';
          ...
          {activeTab === 'profile' ? (
            ...
            <div className="max-w-5xl space-y-8">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {/* Production Cloud Database Card */}
                  <div className="space-y-4 p-6 bg-slate-50 border border-slate-200 rounded-[2rem] flex flex-col justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="font-bold text-slate-900 text-sm text-left">Production Real-Time Database</h4>
                      </div>
                      <p className="text-xs text-slate-500 font-medium text-left leading-relaxed">
                        Your ERP workspace is live and synchronized in real time with Google Cloud Firestore. Multi-tenant company isolation, persistent transactions, and live listeners are active across all branches.
                      </p>

                      <div className="mt-4 p-3.5 bg-emerald-50/80 border border-emerald-200 rounded-xl space-y-1.5 text-left">
                        <div className="flex items-center gap-2 text-xs font-bold text-emerald-900">
                          <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                          <span>Firestore Connected • Production Active</span>
                        </div>
                        <div className="text-[11px] text-slate-600 pl-6 font-mono break-all">
                          DB ID: {firebaseConfig.firestoreDatabaseId}
                        </div>
                      </div>
                    </div>

                    <div className="pt-4 border-t border-slate-200/60 flex items-center justify-between text-xs text-slate-500 font-semibold">
                      <span className="flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        Real-Time Sync Online
                      </span>
                      <span className="font-mono text-[10px] text-slate-400">SSL 256-bit</span>
                    </div>
                  </div>
               </div>
